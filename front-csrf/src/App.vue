...
<main>
  ...
  <section>
    <h2>前提知識</h2>
    <ul>
      <li>Cookieはポート番号では分離されないため、http://localhostもhttp://localhost:8888も同一ドメインと判断する</li>
      <li>Cookieは「ドメインをまたぐリクエストにも自動で付与される」という挙動</li>
  ...
  <ul>
    ...
    <li>プリフライト時のhttpメソッドはOPTIONなので、このリクエスト時にオリジンをチェックすれば許可していないサイトからのリクエストを弾くことができる</li>
  </ul>
  <h2>Cookie SameSite属性</h2>
  <ul>
    <li><b>（SameSite=Lax）</b>外部サイトからのGETメソッドでの遷移時(Top Level Navigation)のみ、Cookieを送信する。
これにより、Webサイトの使いやすさとセキュリティを両立させることができる</li>
...
.memo{
  background-color: rgb(199, 242, 245);
  padding: 20px;
  font-size: 16px;
  border-radius: 4px;
  list-style: none;
